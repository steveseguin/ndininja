...
def main():
    ...
    sub = Popen(["pyinstaller", "--clean", "--onefile", "--windowed", "--icon", "images/stagecast.ico", "pyinstaller.spec"], env=env)
    sub.communicate()
    ...
    cefapp_dir = os.path.join(curdir, "dist", "stagecast")
    executable = os.path.join(cefapp_dir, "stagecast"+EXE_EXT)
    ...
    # run the result binary using "cmd.exe /k stagecast.exe", so that
